...
// My function
static void print_nonfreed_blocks(item *list);
...
void fini(void)
{
  ...
  LOG_STATISTICS(n_allocb, (n_allocb)? (n_allocb/(n_malloc+n_calloc+n_realloc)) : 0L, n_freeb);

  // Check whether Non-deallocated blocks exist
  if (n_allocb > n_freeb) {
    LOG_NONFREED_START();
    print_nonfreed_blocks(list);
  }
